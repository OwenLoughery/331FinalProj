...
Theoretically, if the simulated data were exactly the same as the observed data, the relationship would be modeled by the line y = x. We can see that the fitted (blue) linear regression line is approximately the line y = x (red). Thus, while they are not exactly the same, the simulated values for mean number of babies are similar to the observed values for mean number of babies. 

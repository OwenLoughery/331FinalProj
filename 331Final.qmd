...
We decided to create a scatterplot to visualize the potential relationship between the Mean Years in School and Babies per Woman. 
```{r}
school_fertility_df |>
...
When we visualize two quantitative variables with a scatterplot, we can perform a linear regression, and view the overall trend of our data with a line of best fit. In this dataset, we can see that the line of best fit has a negative slope, supporting our aforementioned hypothesis of a negative relationship between the two variables. Another key component of the line of best fit is the y-intercept: in this model, if a given country's women in a given year have 0 average years in school, we'd expect the amount of babies per woman to be the y-intercept of the graph.
...
Based on the regression above and the parameter estimates, the slope is estimated to be 0.46. This means that for every increase in one year of school, babies per woman  is expected to decrease by 0.46 babies on average. Additionally, with an estimated intercept value of 7.09, for a given women in a country in a given year with 0 average years in school, the expected number of babies per woman in that year is 7.09, approximately 7.
...
Our simulated R² values seem to follow a normal distribution, being centered at .55. This means that our simulated data, on average, has 55% of the variability in babies per woman being explained by the variability in mean years in school. Since our actual R² value in our observed data was .7453, we can say that our simulated model does not fully capture the relationship between the two variables in our dataset.
